...
class RubricEngineTest {
    ...
    @Test
    void shouldReturnZeroScoreWhenEvaluatorReturnsZero() {
        var engine = engineWithFixedScore(0.0);

        RubricEvaluation eval =
                engine.evaluate(rubric(criterion("c1", 1.0)), NodeResult.empty(), new HashMap<>());
    ...
    @Test
    void shouldReturnPerfectScoreWhenEvaluatorReturnsHundred() {
        var engine = engineWithFixedScore(100.0);
    ...
    @Test
    void shouldApplyWeightsProportionally() {
        ...
        RubricEvaluation eval =
                engine.evaluate(
                        rubric(criterion("c1", 3.0), criterion("c2", 1.0)),
                        NodeResult.empty(),
                        new HashMap<>());
    ...
    void shouldPassAtExactThreshold() {
        var engine = engineWithFixedScore(70.0);
    ...
    void shouldFailWhenScoreJustBelowThreshold() {
        var engine = engineWithFixedScore(69.9);
    ...
    @Test
    void shouldReturnZeroWhenAllCriteriaHaveZeroWeight() {
        var engine = engineWithFixedScore(100.0);

        RubricEvaluation eval =
                engine.evaluate(rubric(criterion("c1", 0.0)), NodeResult.empty(), new HashMap<>());
    ...
    @Test
    void shouldIncludePerCriterionDetailInEvaluation() {
        var engine = engineWithFixedScore(85.0);
