#!/usr/bin/env bash
# install-dev.sh — Hensu CLI local-build installer (dev/test only)
#
# Usage:
#   bash hensu-cli/scripts/install-dev.sh [--prefix ~/.local] [--jar PATH] [--no-service]
#
# Options:
...
#   1. Verifies Java 25+ is available
#   2. Copies the locally built Quarkus app directory to $HENSU_HOME/lib/quarkus-app/
#   3. Writes a 'hensu' launcher script to $PREFIX/bin/hensu
#   4. Optionally installs a platform service unit so the daemon starts on login:
...
#       For production installs use install.sh instead.
...
# Resolve default JAR path relative to this script's location
...
RUNNER_JAR="${APP_DIR}/quarkus-run.jar"

# ——— Step 1: Java version check ——————————————————————————————————————————————

printf "\n$(bold 'Hensu CLI installer') ${YELLOW}[dev/local build]${RESET}\n\n"
...
# ——— Step 3: Copy quarkus-app directory ——————————————————————————————————————
...
cat > "${WRAPPER}" <<EOF
#!/usr/bin/env bash
# Hensu CLI launcher — generated by install-dev.sh (local build)
# Do not edit: re-run install-dev.sh to update.

HENSU_HOME="\${HENSU_HOME:-\${HOME}/.hensu}"
export HENSU_JAR="\${HENSU_HOME}/lib/quarkus-app/quarkus-run.jar"
...
[Unit]
Description=Hensu Workflow Daemon (dev build)
...
printf "\n  ${GREEN}✓ Hensu dev build installed!${RESET}\n\n"
printf "  Quick start:\n"
