...
- Never `instanceof` on `TransitionRule` subtypes outside `io.hensu.core.workflow.transition`, the serializers, and the visualizers – consume `requiredRoutingVars()` (engine-variable wiring) or `trigger()` (inner-rule evaluation/labeling) so decorators keep their semantics.
